...
fn main() {
    ...
    std::thread::spawn(move || {
        ...
        use crate::devices::connect_compatible_device;
        ...
        .arg(
            Arg::new("press_mute_key")
                .long("press_mute_key")
                .required(false)
                .help("The app will simulate pressing the microphone mute key whoever the controller is muted or unmuted.")
                .default_value("true")
        ...
        loop {
            ...
            loop {
                ...
                for command in first.into_iter().chain(rx.try_iter()) {
                    ...
                    std::thread::sleep(crate::devices::RESPONSE_DELAY);
