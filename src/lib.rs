...
pub const APP_NAME: &str = "open-steam-controller";
pub const APP_NAME_PRETTY: &str = "Open Steam Controller";
pub const UDEV_RULE_PATH_SYSTEM: &str = formatc!("/etc/udev/rules.d/99-{}.rules", APP_NAME);
pub const UDEV_RULE_PATH_USER: &str = formatc!("/usr/lib/udev/rules.d/99-{}.rules", APP_NAME);
pub const UDEV_RULES: &str = include_str!("./../99-open-steam-controller.rules");
...
pub fn act_as_askpass_handler() -> ! {
    let a = dialog::Password::new(format!(
        "Created rule at /usr/lib/udev/rules.d/99-{}.rules",
        APP_NAME
    ))
...
pub fn update_rule(path: &str, rules: &str) {
    ...
    match status {
        Ok(exit_status) if exit_status.success() => {
            show_message(&format!("created rule at {path}.\nYou may need to replug your controller for the udev rules to take effect."));
        }
        Ok(e) => {
            show_message(&format!("Failed to create rule at {path}: {}.\nYour controller may not be recognized without the correct udev rules.", e));
        }
        Err(e) => {
            show_message(&format!("Failed to create rule at {path}: {}\nYour controller may not be recognized without the correct udev rules.", e));
        }
    ...
    match (user_rule_state, system_rule_state) {
        ...
        (RuleState::RuleMatch(false), _) | (RuleState::RuleExists(true), _) => {
            handle_udev_rule_user_interaction(UDEV_RULE_PATH_USER,
                ...
                "Your controller may not be recognized without the correct udev rules.");
        }
        ...
        (RuleState::RuleExists(false), RuleState::RuleExists(false)) => {
            handle_udev_rule_user_interaction(
                ...
                "Without udev rules your controller can only be accessed when running as root.",
            );
        }
    }
}
